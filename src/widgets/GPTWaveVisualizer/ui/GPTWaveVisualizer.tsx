...
import styles from "./GPTWaveVisualizer.module.scss";
...
export function GPTWaveVisualizer({ volume, isInterviewing }: Props) {
  ...
  useEffect(() => {
    ...
    const lines = svgRef.current.querySelectorAll("line, path, polyline");
    linesRef.current = Array.from(lines) as SVGLineElement[];
  ...
  useEffect(() => {
    const scale = volume > 10 ? 1 + volume / 200 : 1;
    const opacity = volume > 10 ? 1 : 0.4;
  ...
  const firstStopColor = isInterviewing ? "#35AAF9" : "#993BDC";
  const secondStopColor = isInterviewing ? "#993BDC" : "#35AAF9";
  ...
  return (
    ...
    >
      ...
      >
        <stop offset="0" stopColor={firstStopColor} stopOpacity="1" />
        <stop offset="1" stopColor={secondStopColor} />
      </linearGradient>
      <g
        id="bg"
        fill="none"
