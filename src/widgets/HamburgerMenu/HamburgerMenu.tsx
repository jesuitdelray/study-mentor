...
export function HamburgerMenu({
  ...
  return (
    <div className={styles.container}>
      <NormalButton onClick={toggle} className={styles.burgerButton} variant="tertiary" size="large">
        <SettingsIcon
          style={{
      ...
      <div className={clsx(styles.menuItems, isOpen && styles.isOpen)}>
        <div className={styles.volumeContainer} onClick={() => setIsAllowedVolume((prev: boolean) => !prev)}>
          <Typography className={styles.volumeTitle} variant="body-1" color="greyscale500">
            Allow Volume
          </Typography>
        ...
        <Input value={topic} onChange={(e) => setTopic(e.target.value)} placeholder="Topic" className={styles.input} />
        <Input
          value={position}
        ...
        />
        <Input value={token} onChange={(e) => setToken(e.target.value)} placeholder="Token" className={styles.input} />
        <div className={styles.buttonsContainer}>
          <NormalButton onClick={onSave} className={styles.saveButton} size="large">
            Save
          </NormalButton>
          <NormalButton onClick={onClearAll} className={styles.clearAllButton} variant="secondary" size="large">
            Clear all
          </NormalButton>
        </div>
      </div>
